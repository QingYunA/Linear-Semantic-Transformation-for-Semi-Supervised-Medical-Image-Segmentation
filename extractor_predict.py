...
def predict(model, config, logger):
    ...
    model,extractor = accelerator.prepare(model,extractor)
    # start progess
    progress.start()
    for i, item in enumerate(dataset):
        ...
        patch_loader = torch.utils.data.DataLoader(
            grid_sampler, batch_size=config.batch_size, shuffle=False, num_workers=0, pin_memory=True,drop_last=True
        )
        ...
        with torch.no_grad():
            for j, batch in enumerate(patch_loader):
                ...
                pred = model(x)
                extractor_pred = extractor(fea_hooks)

                # mask = torch.sigmoid(pred.clone())
                # mask[mask > 0.5] = 1
                # mask[mask <= 0.5] = 0
                mask = extractor_pred[-1].clone()
